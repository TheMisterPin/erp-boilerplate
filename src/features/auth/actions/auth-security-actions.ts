...
export async function resetPasswordAction(input: unknown): Promise<ActionResult<true>> {
 return withErrorBoundary(async () => { const data = resetPasswordSchema.parse(input), now = new Date(), tokenHash = hashPasswordResetToken(data.token), password = await hashPassword(data.newPassword); await prisma.$transaction(async tx => { const reset = await tx.passwordResetToken.findFirst({ where: { tokenHash, usedAt: null, expiresAt: { gt: now } }, select: { id: true, userId: true, user: { select: { isActive: true, deletedAt: true } } } }); if (!reset || !reset.user.isActive || reset.user.deletedAt) throw invalidReset(); const consumed = await tx.passwordResetToken.updateMany({ where: { id: reset.id, usedAt: null, expiresAt: { gt: now } }, data: { usedAt: now } }); if (consumed.count !== 1) throw invalidReset(); await tx.user.update({ where: { id: reset.userId }, data: { password, sessionVersion: { increment: 1 } } }); await tx.passwordResetToken.deleteMany({ where: { userId: reset.userId, id: { not: reset.id } } }); await logActivity({ userId: reset.userId, activity: "PASSWORD_RESET" }, tx) }); await clearSession(); return true as const })
}
export async function revokeSessionsAction(): Promise<ActionResult<true>> { return withErrorBoundary(async () => { const session = await requireSession(); await prisma.$transaction(async tx => { await tx.user.update({ where: { id: session.userId }, data: { sessionVersion: { increment: 1 } } }); await tx.passwordResetToken.deleteMany({ where: { userId: session.userId } }); await logActivity({ userId: session.userId, activity: "SESSION_REVOKED" }, tx) }); await clearSession(); return true as const }) }
