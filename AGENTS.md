...
- **Testing**: Vitest under `tests/` (unit / components / integration). Tenant-owned verticals extend `tests/integration/tenant-isolation.integration.test.ts` in the same PR. Do not import `src/lib/db.ts` from tests — use `tests/support`. See `.docs/components/testing.md`.
